...
def compareApkFromBundle(bundle, apk):
    ...
    firstList = list(
        filter(lambda info: info.filename not in FILES_TO_IGNORE, apkZip.infolist())
    )
    secondList = list(
        filter(
            lambda secondInfo: secondInfo.filename not in FILES_TO_IGNORE,
            bundleZip.infolist(),
        )
    )
...
if __name__ == "__main__":
    ...
    if (
        ...
        or compareApkFromBundle(sys.argv[1], sys.argv[2]) == True
    ):
